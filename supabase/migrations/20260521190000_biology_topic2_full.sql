...
-- ============================================================================
-- QUIZZES — 6 questions each
-- ============================================================================

-- Wipe any prior quiz for these 2 lessons
...
where slug = 'quiz' and lesson_id in (
  ...
  where s.slug = 'biology' and tp.slug = 'classification-diversity'
);
...
join public.subjects s on s.id = tp.subject_id
join (values
  ('classification-binomial-naming',
   'Classification — quick check',
   'Six questions on the seven-level hierarchy, binomial naming and using a dichotomous key.'),
  ('diversity-of-organisms',
   'Diversity — quick check',
   'Six questions on the five kingdoms, viruses, plants and the main animal groups.')
) as a(lesson_slug, title, instructions)
  on a.lesson_slug = l.slug
where s.slug = 'biology' and tp.slug = 'classification-diversity';
...
with quiz as (
  ...
  join public.subjects s on s.id = tp.subject_id
  where s.slug = 'biology' and tp.slug = 'classification-diversity'
    and l.slug = 'classification-binomial-naming' and a.slug = 'quiz'
)
...
from quiz cross join (values
  ('mcq', 'Which of these is the SMALLEST group in the classification hierarchy?', 1, 10,
   '[{"id":"a","text":"Kingdom"},{"id":"b","text":"Class"},{"id":"c","text":"Family"},{"id":"d","text":"Species"}]',
   '"d"', null, null, null,
   'Kingdom → Phylum → Class → Order → Family → Genus → Species. Species is the smallest.'),
  ('mcq', 'Which of these is written correctly?', 1, 20,
   '[{"id":"a","text":"Homo Sapiens"},{"id":"b","text":"homo sapiens"},{"id":"c","text":"*Homo sapiens*"},{"id":"d","text":"HOMO SAPIENS"}]',
   '"c"', null, null, null,
   'Binomial name: Genus capitalised, species lowercase, both italicised (or underlined when hand-written).'),
  ('true_false', 'Two organisms are in the same species if they can breed together and produce fertile offspring.', 1, 30,
   null, 'true', null, null, null,
   'That is the standard definition of a species used in NSSCO Biology.'),
  ('fill_in', 'A series of yes/no questions used to identify an unknown organism is called a ____ key.', 1, 40,
   null, '["dichotomous"]', null, null, false,
   '"Dichotomous" means "split in two" — each question has two possible answers.'),
  ('mcq', 'In the binomial name *Loxodonta africana* (African elephant), which word is the genus?', 1, 50,
   '[{"id":"a","text":"Loxodonta"},{"id":"b","text":"africana"},{"id":"c","text":"Both"},{"id":"d","text":"Neither — it''s the family"}]',
   '"a"', null, null, null,
   'The first (capitalised) word is always the genus; the second (lowercase) is the species.'),
  ('mcq', 'Why do scientists use Latin binomial names instead of common names like "robin"?', 2, 60,
   '[{"id":"a","text":"Latin sounds more scientific"},{"id":"b","text":"Common names vary between countries and languages — a binomial name uniquely identifies one species"},{"id":"c","text":"Latin names are shorter"},{"id":"d","text":"Latin is the official language of biology"}]',
   '"b"', null, null, null,
   'A "robin" in Europe is different from a "robin" in Africa. *Erithacus rubecula* is only one species.')
) as q(type, prompt, points, sort_order, options, correct, tolerance, unit, case_sensitive, explanation);


-- Diversity quiz questions
with quiz as (
  select a.id as activity_id from public.activities a
  join public.lessons l on l.id = a.lesson_id
  join public.topics tp on tp.id = l.topic_id
  join public.subjects s on s.id = tp.subject_id
  where s.slug = 'biology' and tp.slug = 'classification-diversity'
    and l.slug = 'diversity-of-organisms' and a.slug = 'quiz'
)
insert into public.questions
  (activity_id, type, prompt, points, sort_order, options, correct, tolerance, unit, case_sensitive, explanation)
select quiz.activity_id, q.type, q.prompt, q.points, q.sort_order,
       q.options::jsonb, q.correct::jsonb, q.tolerance::numeric, q.unit::text, q.case_sensitive::boolean, q.explanation
from quiz cross join (values
  ('mcq', 'Which kingdom do mushrooms belong to?', 1, 10,
   '[{"id":"a","text":"Plants"},{"id":"b","text":"Animals"},{"id":"c","text":"Fungi"},{"id":"d","text":"Bacteria"}]',
   '"c"', null, null, null,
   'Fungi have cell walls made of chitin (not cellulose) and no chloroplasts. They feed by decomposing dead matter.'),
  ('true_false', 'A virus is classified in the bacteria kingdom.', 1, 20,
   null, 'false', null, null, null,
   'Viruses are not put in any kingdom — most biologists don''t consider them truly alive.'),
  ('mcq', 'An animal has 8 legs and 2 body parts (no wings). What group is it in?', 1, 30,
   '[{"id":"a","text":"Insect"},{"id":"b","text":"Arachnid"},{"id":"c","text":"Crustacean"},{"id":"d","text":"Myriapod"}]',
   '"b"', null, null, null,
   'Arachnids (spiders, scorpions, ticks) have 8 legs and 2 body parts. Insects have 6 legs and 3 body parts.'),
  ('mcq', 'Which of the following is a feature of MAMMALS that is NOT shared with birds?', 1, 40,
   '[{"id":"a","text":"Warm-blooded"},{"id":"b","text":"Have a backbone"},{"id":"c","text":"Give birth to live young and feed milk"},{"id":"d","text":"Lay eggs"}]',
   '"c"', null, null, null,
   'Both are warm-blooded vertebrates, but only mammals give birth to live young and produce milk. (Birds and most reptiles lay eggs.)'),
  ('fill_in', 'A maize plant has one seed leaf in its seedling. Maize is therefore a ____.', 1, 50,
   null, '["monocot","monocotyledon"]', null, null, false,
   'Monocots have one seed leaf and parallel leaf veins. Examples: maize, grass, palm, lily.'),
  ('mcq', 'Frogs are amphibians because:', 2, 60,
   '[{"id":"a","text":"They have scales and lay eggs on land"},{"id":"b","text":"They have moist skin and lay eggs in water, but adults can live on land"},{"id":"c","text":"They are warm-blooded with fur"},{"id":"d","text":"They have feathers"}]',
   '"b"', null, null, null,
   'Amphibians (frogs, toads) have moist skin, lay eggs in water, and the young develop in water before metamorphosing.')
) as q(type, prompt, points, sort_order, options, correct, tolerance, unit, case_sensitive, explanation);
